...
GATE_CMD = "mhc"
HC_API = "http://72.61.18.119:7009/shopify?cc={cc}"
active_sessions = {}
...
def process_single_card(card):
    try:
        resp = requests.get(HC_API.format(cc=card), timeout=90, verify=False)
        if resp.status_code == 200:
            data = resp.json()
...
async def msh_command(message: types.Message):
    ...
    for idx, card in enumerate(raw_cards):
        ...
        # DON'T clear response - keep previous response visible
        ...
        # Update live message with the result
        text, kb = build_live_message(session_data)
        try:
            await msg.edit_text(text, parse_mode="HTML", reply_markup=kb)
        except:
        ...
        # Wait 3-5 seconds so user can read the response before next card
...
@router.callback_query(F.data.startswith("msh_card_"))
async def msh_card_callback(callback: types.CallbackQuery):
...
@router.callback_query(F.data.startswith("msh_response_"))
async def msh_response_callback(callback: types.CallbackQuery):
    user_id = int(callback.data.split("_")[2])
    if user_id != callback.from_user.id:
        await callback.answer("❌ Not your session!", show_alert=True)
        return
...
# Override the approved message with buy button
# The approved_msg is already defined in the main loop, we need to add the button there
# But since we can't easily override that, we'll add a keyboard to the reply

# We'll modify the approved message sending part in the loop
# Actually let's add the button to the approved message itself by modifying the inline keyboard
# The approved_msg is sent as a separate message, we can add reply_markup to it

# Since the approved_msg is sent with await message.answer(approved_msg, parse_mode="HTML", reply_markup=get_buy_keyboard())
# We need to add reply_markup to that call
# Let me just append a note about it

def get_buy_keyboard():
    """Get inline keyboard with buy button"""
