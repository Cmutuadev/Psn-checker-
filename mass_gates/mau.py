...
GATE_NAME = "MAU"
GATE_CMD = "mau"
MAU_API = "http://5.83.136.66:8000/mau?cc={cc}"
active_sessions = {}
...
def process_single_card(card):
    try:
        resp = requests.get(MAU_API.format(cc=card), timeout=90, verify=False)
        if resp.status_code == 200:
            data = resp.json()
...
@router.message(Command("mau"))
async def mau_command(message: types.Message):
...
@router.callback_query(F.data.startswith("mau_card_"))
async def mau_card_callback(callback: types.CallbackQuery):
...
@router.callback_query(F.data.startswith("mau_response_"))
async def mau_response_callback(callback: types.CallbackQuery):
    user_id = int(callback.data.split("_")[2])
    if user_id != callback.from_user.id:
        await callback.answer("❌ Not your session!", show_alert=True)
        return
...
@router.callback_query(F.data.startswith("mau_stop_"))
async def mau_stop_callback(callback: types.CallbackQuery):
    user_id = int(callback.data.split("_")[2])
    if user_id != callback.from_user.id:
        await callback.answer("❌ Not your session!", show_alert=True)
        return
...
@router.callback_query(F.data.startswith("mau_pause_"))
async def mau_pause_callback(callback: types.CallbackQuery):
    user_id = int(callback.data.split("_")[2])
    if user_id != callback.from_user.id:
        await callback.answer("❌ Not your session!", show_alert=True)
        return
