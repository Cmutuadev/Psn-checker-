...
GATE_NAME = "NMI2"
GATE_CMD = "mnmi2"
MNMI2_API = "http://5.83.136.66:8001/nmi2?cc={cc}"
active_sessions = {}
...
def process_single_card(card):
    try:
        resp = requests.get(MNMI2_API.format(cc=card), timeout=90, verify=False)
        if resp.status_code == 200:
            data = resp.json()
...
@router.message(Command("mnmi2"))
async def mnmi2_command(message: types.Message):
...
@router.callback_query(F.data.startswith("mnmi2_card_"))
async def mnmi2_card_callback(callback: types.CallbackQuery):
...
@router.callback_query(F.data.startswith("mnmi2_response_"))
async def mnmi2_response_callback(callback: types.CallbackQuery):
    user_id = int(callback.data.split("_")[2])
    if user_id != callback.from_user.id:
        await callback.answer("❌ Not your session!", show_alert=True)
        return
...
@router.callback_query(F.data.startswith("mnmi2_stop_"))
async def mnmi2_stop_callback(callback: types.CallbackQuery):
    user_id = int(callback.data.split("_")[2])
    if user_id != callback.from_user.id:
        await callback.answer("❌ Not your session!", show_alert=True)
        return
...
@router.callback_query(F.data.startswith("mnmi2_pause_"))
async def mnmi2_pause_callback(callback: types.CallbackQuery):
    user_id = int(callback.data.split("_")[2])
    if user_id != callback.from_user.id:
        await callback.answer("❌ Not your session!", show_alert=True)
        return
