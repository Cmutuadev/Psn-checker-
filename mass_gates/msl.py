...
GATE_NAME = "StripLink"
GATE_CMD = "msl"
MSL_API = "http://5.83.136.66:8001/striplink?cc={cc}"
active_sessions = {}
...
def process_single_card(card):
    try:
        resp = requests.get(MSL_API.format(cc=card), timeout=90, verify=False)
        if resp.status_code == 200:
            data = resp.json()
...
@router.message(Command("msl"))
async def msl_command(message: types.Message):
...
@router.callback_query(F.data.startswith("msl_card_"))
async def msl_card_callback(callback: types.CallbackQuery):
...
@router.callback_query(F.data.startswith("msl_response_"))
async def msl_response_callback(callback: types.CallbackQuery):
    user_id = int(callback.data.split("_")[2])
    if user_id != callback.from_user.id:
        await callback.answer("❌ Not your session!", show_alert=True)
        return
...
@router.callback_query(F.data.startswith("msl_stop_"))
async def msl_stop_callback(callback: types.CallbackQuery):
    user_id = int(callback.data.split("_")[2])
    if user_id != callback.from_user.id:
        await callback.answer("❌ Not your session!", show_alert=True)
        return
...
@router.callback_query(F.data.startswith("msl_pause_"))
async def msl_pause_callback(callback: types.CallbackQuery):
    user_id = int(callback.data.split("_")[2])
    if user_id != callback.from_user.id:
        await callback.answer("❌ Not your session!", show_alert=True)
        return
